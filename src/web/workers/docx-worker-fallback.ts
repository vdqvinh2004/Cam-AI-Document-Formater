...
export async function inspectDocxFallback(source: ArrayBuffer): Promise<DocxInspection> {
...
export async function formatDocxFallback(source: ArrayBuffer, plan: { version: number; operations: BrowserFormattingOperation[] }): Promise<Blob> {
  if (source.byteLength === 0 || source.byteLength > MAX_PACKAGE_BYTES) {
    throw new Error('DOCX is empty or exceeds the size limit.');
  }
...
export async function extractDocxTextFallback(source: ArrayBuffer): Promise<string> {
  if (source.byteLength === 0 || source.byteLength > MAX_PACKAGE_BYTES) {
    return '';
  }
